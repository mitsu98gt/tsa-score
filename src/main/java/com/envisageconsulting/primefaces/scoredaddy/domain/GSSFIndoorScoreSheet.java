package com.envisageconsulting.primefaces.scoredaddy.domain;

public class GSSFIndoorScoreSheet {

	public String name;
	public String date;
	public String model;
	public String entry;
	public String checkStock;
	public String checkUnlimited;
	public String checkPocket;
	public String checkWoman;
	public String checkSenior;
	public String checkJunior;
	public String targetOneX;
	public String targetOneTen;
	public String targetOneEight;
	public String targetOneFive;
	public String targetOneMisses;
	public String targetOneTotal;
	public String targetTwoX;
	public String targetTwoTen;
	public String targetTwoEight;
	public String targetTwoFive;
	public String targetTwoMisses;
	public String targetTwoTotal;
	public String sumX;
	public String sumTen;
	public String sumEight;
	public String sumFive;
	public String sumMisses;
	public String sumTotal;
	public String totalX;
	public String totalTen;
	public String totalEight;
	public String totalFive;
	public String penalty;
	public String finalScore;
	public String rangeOfficerInitials;
	public String competitorInitials;
	
	/**
	 * @return the name
	 */
	public String getName() {
		return name;
	}
	/**
	 * @param name the name to set
	 */
	public void setName(String name) {
		this.name = name;
	}
	/**
	 * @return the date
	 */
	public String getDate() {
		return date;
	}
	/**
	 * @param date the date to set
	 */
	public void setDate(String date) {
		this.date = date;
	}
	/**
	 * @return the model
	 */
	public String getModel() {
		return model;
	}
	/**
	 * @param model the model to set
	 */
	public void setModel(String model) {
		this.model = model;
	}
	/**
	 * @return the entry
	 */
	public String getEntry() {
		return entry;
	}
	/**
	 * @param entry the entry to set
	 */
	public void setEntry(String entry) {
		this.entry = entry;
	}
	/**
	 * @return the checkStock
	 */
	public String getCheckStock() {
		return checkStock;
	}
	/**
	 * @param checkStock the checkStock to set
	 */
	public void setCheckStock(String checkStock) {
		this.checkStock = checkStock;
	}
	/**
	 * @return the checkUnlimited
	 */
	public String getCheckUnlimited() {
		return checkUnlimited;
	}
	/**
	 * @param checkUnlimited the checkUnlimited to set
	 */
	public void setCheckUnlimited(String checkUnlimited) {
		this.checkUnlimited = checkUnlimited;
	}
	/**
	 * @return the checkPocket
	 */
	public String getCheckPocket() {
		return checkPocket;
	}
	/**
	 * @param checkPocket the checkPocket to set
	 */
	public void setCheckPocket(String checkPocket) {
		this.checkPocket = checkPocket;
	}
	/**
	 * @return the checkWoman
	 */
	public String getCheckWoman() {
		return checkWoman;
	}
	/**
	 * @param checkWoman the checkWoman to set
	 */
	public void setCheckWoman(String checkWoman) {
		this.checkWoman = checkWoman;
	}
	/**
	 * @return the checkSenior
	 */
	public String getCheckSenior() {
		return checkSenior;
	}
	/**
	 * @param checkSenior the checkSenior to set
	 */
	public void setCheckSenior(String checkSenior) {
		this.checkSenior = checkSenior;
	}
	/**
	 * @return the checkJunior
	 */
	public String getCheckJunior() {
		return checkJunior;
	}
	/**
	 * @param checkJunior the checkJunior to set
	 */
	public void setCheckJunior(String checkJunior) {
		this.checkJunior = checkJunior;
	}
	/**
	 * @return the targetOneX
	 */
	public String getTargetOneX() {
		return targetOneX;
	}
	/**
	 * @param targetOneX the targetOneX to set
	 */
	public void setTargetOneX(String targetOneX) {
		this.targetOneX = targetOneX;
	}
	/**
	 * @return the targetOneTen
	 */
	public String getTargetOneTen() {
		return targetOneTen;
	}
	/**
	 * @param targetOneTen the targetOneTen to set
	 */
	public void setTargetOneTen(String targetOneTen) {
		this.targetOneTen = targetOneTen;
	}
	/**
	 * @return the targetOneEight
	 */
	public String getTargetOneEight() {
		return targetOneEight;
	}
	/**
	 * @param targetOneEight the targetOneEight to set
	 */
	public void setTargetOneEight(String targetOneEight) {
		this.targetOneEight = targetOneEight;
	}
	/**
	 * @return the targetOneFive
	 */
	public String getTargetOneFive() {
		return targetOneFive;
	}
	/**
	 * @param targetOneFive the targetOneFive to set
	 */
	public void setTargetOneFive(String targetOneFive) {
		this.targetOneFive = targetOneFive;
	}
	/**
	 * @return the targetOneMisses
	 */
	public String getTargetOneMisses() {
		return targetOneMisses;
	}
	/**
	 * @param targetOneMisses the targetOneMisses to set
	 */
	public void setTargetOneMisses(String targetOneMisses) {
		this.targetOneMisses = targetOneMisses;
	}
	/**
	 * @return the targetOneTotal
	 */
	public String getTargetOneTotal() {
		return targetOneTotal;
	}
	/**
	 * @param targetOneTotal the targetOneTotal to set
	 */
	public void setTargetOneTotal(String targetOneTotal) {
		this.targetOneTotal = targetOneTotal;
	}
	/**
	 * @return the targetTwoX
	 */
	public String getTargetTwoX() {
		return targetTwoX;
	}
	/**
	 * @param targetTwoX the targetTwoX to set
	 */
	public void setTargetTwoX(String targetTwoX) {
		this.targetTwoX = targetTwoX;
	}
	/**
	 * @return the targetTwoTen
	 */
	public String getTargetTwoTen() {
		return targetTwoTen;
	}
	/**
	 * @param targetTwoTen the targetTwoTen to set
	 */
	public void setTargetTwoTen(String targetTwoTen) {
		this.targetTwoTen = targetTwoTen;
	}
	/**
	 * @return the targetTwoEight
	 */
	public String getTargetTwoEight() {
		return targetTwoEight;
	}
	/**
	 * @param targetTwoEight the targetTwoEight to set
	 */
	public void setTargetTwoEight(String targetTwoEight) {
		this.targetTwoEight = targetTwoEight;
	}
	/**
	 * @return the targetTwoFive
	 */
	public String getTargetTwoFive() {
		return targetTwoFive;
	}
	/**
	 * @param targetTwoFive the targetTwoFive to set
	 */
	public void setTargetTwoFive(String targetTwoFive) {
		this.targetTwoFive = targetTwoFive;
	}
	/**
	 * @return the targetTwoMisses
	 */
	public String getTargetTwoMisses() {
		return targetTwoMisses;
	}
	/**
	 * @param targetTwoMisses the targetTwoMisses to set
	 */
	public void setTargetTwoMisses(String targetTwoMisses) {
		this.targetTwoMisses = targetTwoMisses;
	}
	/**
	 * @return the targetTwoTotal
	 */
	public String getTargetTwoTotal() {
		return targetTwoTotal;
	}
	/**
	 * @param targetTwoTotal the targetTwoTotal to set
	 */
	public void setTargetTwoTotal(String targetTwoTotal) {
		this.targetTwoTotal = targetTwoTotal;
	}
	/**
	 * @return the sumX
	 */
	public String getSumX() {
		return sumX;
	}
	/**
	 * @param sumX the sumX to set
	 */
	public void setSumX(String sumX) {
		this.sumX = sumX;
	}
	/**
	 * @return the sumTen
	 */
	public String getSumTen() {
		return sumTen;
	}
	/**
	 * @param sumTen the sumTen to set
	 */
	public void setSumTen(String sumTen) {
		this.sumTen = sumTen;
	}
	/**
	 * @return the sumEight
	 */
	public String getSumEight() {
		return sumEight;
	}
	/**
	 * @param sumEight the sumEight to set
	 */
	public void setSumEight(String sumEight) {
		this.sumEight = sumEight;
	}
	/**
	 * @return the sumFive
	 */
	public String getSumFive() {
		return sumFive;
	}
	/**
	 * @param sumFive the sumFive to set
	 */
	public void setSumFive(String sumFive) {
		this.sumFive = sumFive;
	}
	/**
	 * @return the sumMisses
	 */
	public String getSumMisses() {
		return sumMisses;
	}
	/**
	 * @param sumMisses the sumMisses to set
	 */
	public void setSumMisses(String sumMisses) {
		this.sumMisses = sumMisses;
	}
	/**
	 * @return the sumTotal
	 */
	public String getSumTotal() {
		return sumTotal;
	}
	/**
	 * @param sumTotal the sumTotal to set
	 */
	public void setSumTotal(String sumTotal) {
		this.sumTotal = sumTotal;
	}
	/**
	 * @return the totalX
	 */
	public String getTotalX() {
		return totalX;
	}
	/**
	 * @param totalX the totalX to set
	 */
	public void setTotalX(String totalX) {
		this.totalX = totalX;
	}
	/**
	 * @return the totalTen
	 */
	public String getTotalTen() {
		return totalTen;
	}
	/**
	 * @param totalTen the totalTen to set
	 */
	public void setTotalTen(String totalTen) {
		this.totalTen = totalTen;
	}
	/**
	 * @return the totalEight
	 */
	public String getTotalEight() {
		return totalEight;
	}
	/**
	 * @param totalEight the totalEight to set
	 */
	public void setTotalEight(String totalEight) {
		this.totalEight = totalEight;
	}
	/**
	 * @return the totalFive
	 */
	public String getTotalFive() {
		return totalFive;
	}
	/**
	 * @param totalFive the totalFive to set
	 */
	public void setTotalFive(String totalFive) {
		this.totalFive = totalFive;
	}
	/**
	 * @return the penalty
	 */
	public String getPenalty() {
		return penalty;
	}
	/**
	 * @param penalty the penalty to set
	 */
	public void setPenalty(String penalty) {
		this.penalty = penalty;
	}
	/**
	 * @return the finalScore
	 */
	public String getFinalScore() {
		return finalScore;
	}
	/**
	 * @param finalScore the finalScore to set
	 */
	public void setFinalScore(String finalScore) {
		this.finalScore = finalScore;
	}
	/**
	 * @return the rangeOfficerInitials
	 */
	public String getRangeOfficerInitials() {
		return rangeOfficerInitials;
	}
	/**
	 * @param rangeOfficerInitials the rangeOfficerInitials to set
	 */
	public void setRangeOfficerInitials(String rangeOfficerInitials) {
		this.rangeOfficerInitials = rangeOfficerInitials;
	}
	/**
	 * @return the competitorInitials
	 */
	public String getCompetitorInitials() {
		return competitorInitials;
	}
	/**
	 * @param competitorInitials the competitorInitials to set
	 */
	public void setCompetitorInitials(String competitorInitials) {
		this.competitorInitials = competitorInitials;
	}
	
}
